...
def get_pickle(pkl_file='simbad_mag_errors.pkl'):
    """ load and unpickle a pickled pandas.DataFrame 'pkl_file' """
    print "Opening %s and unpickling the DataFrame.."%(pkl_file)
    with open(pkl_file,'r') as target:
        df = cPickle.load(target)
    print "..Done"
    return df


def merge_df(df,df_error,mergeOn=['hip','tycho2_id']):
    """ add two pandas.DataFrames togethers """
    df = df.merge(df,df_error,on=mergeOn)
    return df


def plotFull(df,df2,list_variables,variable_types,x='J_K',y='M_J'):
    plt.ion()
    print "cutoff at %s"%(args.cutoff)
    plotHRdiag(df,x=x,y=y)
    plt.colorbar()
    plotVariableStars(list_variables,variable_types,x=x,y=y)
...
def plotHRdiag(df,x='J_K',y='M_J',marker='.',color='b'):
    """ plotting of the background stars, making the actual HR diagram. The plot is a 2d histogram, for better readability. Only bins with at least 10 stars a shown. """
    plt.figure()
    print "Plotting background stars.."
    plt.hist2d(df[x].tolist(),df[y].tolist(),(200,200), norm=LogNorm(),cmin=10,alpha=.5)
...
def plotVariableStars(variablesdf,variabletype=['CEP','BCEP','BCEPS','DSCT','SR','SRA','SRB','SRC','SRD','RR','RRAB','RRC','GDOR','SPB','M'],x='J_K',y='M_G',size=40):
    """ Parent function of getVariableStars. Sequencially select 'variableTypes' variable stars and plot them on the HR diagram. """
    markers = ['^','^','^','v','<','<','<','<','<','>','>','>','*','s','p']
    colors = ['b','b','b','y','r','r','r','r','r','c','c','c','m','g','w']
    for i in range(len(variabletype)):
        plt.scatter(variablesdf[x].loc[variablesdf.loc[:,'Type']==variabletype[i]],variablesdf[y].loc[variablesdf.loc[:,'Type']==variabletype[i]],facecolor=colors[i],marker=markers[i], s=size)
        print "plotting %s as %s%s"%(variabletype[i],colors[i],markers[i])
    return


def getVariableStars(df,df2,variabletype=['CEP','BCEP','BCEPS','DSCT','SR','SRA','SRB','SRC','SRD','RR','RRAB','RRC','GDOR','SPB','M']):
    """ Child function fo plotVariableStars. Process the DataFrame to select only stars marked as 'var_type' variable stars """
    areVariables = df2[df2.loc[:,'Type'].isin(variabletype)]
    Variable_df = pd.merge(df,areVariables,how='inner',on=['hip','tycho2_id'])
...
def simbad_makescript(df2):
    ...
    with open('SIMBAD_script.txt','w') as sc:
        ...
        hip_numbers = df2['hip'].loc[np.isnan(df2.loc[:,'hip'])==False].tolist()
        print "Hip identification.."
        for hip_number in hip_numbers:
            sc.write("query id hip %s\n"%(int(hip_number)))
        print "..Done"
        tycho_numbers = df2['tycho2_id'].loc[pd.isnull(df2.loc[:,'tycho2_id'])==False].tolist()
...
def simbad_outputcolumnClean(outputFile='simbad_output_0.1.txt'):
    """ Go through a SIMBAD output file and checks if a column is missing, by splitting every row into a list and adding missing data as NaN as well as replacing '~' with NaN """
...
def simbad_createDataFrame(cleaned_output):
    """ uses the output from 'simbad_outputcolumnClean' to create a pandas.DataFrame """
...
def simbad_output():
    new_out, data = simbad_outputcolumnClean()
    df = simbad_createDataFrame(new_out)
    pickleIt(df)
    return
...
if __name__=="__main__":
    ...
    parser.add_argument('-c','--cutoff', dest='cutoff', action='store', default=0.2, type=float,
                        help="At which percentage in relative parallax error should the data stop being read. Default=%(default).")
    ...
    if os.path.isfile(target) and os.path.isfile(target2):
        ...
        df = import_data(catalog='xmatch_TGAS_Simbad.csv', params=('hip', 'tycho2_id', 'parallax', 'parallax_error', 'phot_g_mean_mag', 'B', 'V', 'J', 'K'), nrows=args.nrows)
        df = data_process(df, catalog='xmatch_TGAS_Simbad.csv', cutoff=args.cutoff)

        df2 = import_data(catalog='xmatch_TGAS_VSX.csv', params=('hip', 'tycho2_id', 'parallax', 'parallax_error', 'Name', 'V', 'Type'), nrows=args.nrows)
        df2 = data_process(df2, catalog='xmatch_TGAS_VSX.csv', cutoff=args.cutoff)

    variable_types = ['CEP','BCEP','BCEPS','DSCT','SR','SRA','SRB','SRC','SRD','RR','RRAB','RRC','GDOR','SPB','M']
    list_variables = getVariableStars(df,df2,variable_types)
    ########################################################
    # Plotting of HR diag and variables stars with errors: #
    ########################################################
    if args.draw is True:
        plotFull(df,df2,list_variables,variable_types,x='J_K',y='M_J')
